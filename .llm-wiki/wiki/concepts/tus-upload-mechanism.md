...
Suitable for files that can be directly downloaded without additional processing (documents, images, zip, etc.).

```
storage/uploads/<id>   ← raw file from TUS
       │
       ▼ copy
storage/completed/<filename>   ← accessible via /storage/completed/<name>
```
...
- Saves IO — no need to copy first before processing
- Original file stays in `uploads/` — can be deleted after processing is complete
...
1. **TUS overhead is not worth it** — A 100KB avatar requires 3 requests (POST, HEAD, PATCH), multipart only needs 1 POST
2. **Different requirements** — Avatar must update DB `user.avatar` + sync session, needs CSRF. TUS just stores the file
3. **Resumable is only useful for large files** — A failed 100KB upload can just be re-uploaded, a 500MB file is a real loss if not resumable
