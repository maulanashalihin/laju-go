...
- **Models**: `app/models/` — Domain structs (entity + DTO). Pure data, no business logic, no DB access. Dipakai oleh Handler (DTO request/response) dan Service (entity) sebagai boundary type.
...
| **Models** | Domain struct (entity + DTO), dipakai cross-layer sebagai boundary type | Business logic, akses DB, import `app/queries` |
...
`app/models/` berisi **pure data struct** — domain entity dan DTO. Tidak boleh ada business logic atau akses DB di sini.

**File convention**:

| File | Isi |
|------|-----|
| `<entity>.go` | Domain struct + `TableName()` + const enum (e.g. `user.go`, `session.go`) |
| `dto.go` | Request/response DTO kumulatif + `ToResponse()` helper |

**Pattern per entity baru**:

```go
// app/models/order.go — contoh
...
- ✅ Satu file per entity (`user.go`, `order.go`, `product.go`)
- ✅ DTO bisa di-split per module kalau `dto.go` udah bloat: `dto_auth.go`, `dto_orders.go`
- ✅ Tambah `ToResponse()` helper di file DTO untuk transformasi entity → response
- ❌ Jangan import `app/queries` — models gak boleh tahu query layer
- ❌ Jangan taruh method dengan business logic (validation, calc, dll) — itu di Service
