...
# Restarts the user service after deploy.sh has uploaded updated artifacts.
...
ssh "$SERVER_USER@$SERVER_HOST" "systemctl --user stop $SERVICE_NAME" || true
...
ssh "$SERVER_USER@$SERVER_HOST" "journalctl --user -u $SERVICE_NAME -n 5 --no-pager"
