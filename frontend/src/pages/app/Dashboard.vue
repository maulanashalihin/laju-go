      ...
      >
        ...
        <div class="relative grid md:grid-cols-2 gap-8 items-center">
          ...
          <ol class="space-y-2">
            ...
            >
              <div v-if="step.done" class="shrink-0 w-6 h-6 rounded-full bg-brand-400/20 flex items-center justify-center">
                <CircleCheck class="w-3.5 h-3.5 text-brand-600 dark:text-brand-400" />
              </div>
              <div v-else class="shrink-0 w-6 h-6 rounded-full border border-neutral-300 dark:border-neutral-700 flex items-center justify-center text-[10px] font-mono text-neutral-500">
                {{ i + 1 }}
              </div>
      ...
      <div class="grid lg:grid-cols-3 gap-5 pt-2">
        ...
        >
          <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-200/80 dark:border-white/[0.04]">
            ...
            <Link href="/app/profile" class="text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 transition-colors inline-flex items-center gap-1">
              View all
          ...
          <ul class="divide-y divide-neutral-200/80 dark:divide-white/[0.04]">
            ...
            >
              <div :class="['shrink-0 w-9 h-9 rounded-lg flex items-center justify-center', eventToneBg(event.tone)]">
                <component :is="event.icon" :class="['w-4 h-4', eventToneText(event.tone)]" />
              </div>
        ...
        >
          ...
          <ul class="divide-y divide-neutral-200/80 dark:divide-white/[0.04]">
            <li
              v-for="(service, i) in systemHealth"
              :key="i"
            ...
            >
              <div class="shrink-0 w-8 h-8 rounded-lg bg-neutral-100/80 dark:bg-neutral-800/50 flex items-center justify-center">
                <component :is="service.icon" class="w-4 h-4 text-neutral-500 dark:text-neutral-400" />
              </div>
              <span class="flex-1 text-sm font-medium text-neutral-700 dark:text-neutral-300">{{ service.label }}</span>
              <span :class="['inline-flex items-center gap-1.5 text-xs font-medium', statusTone(service.status, (service as any).tone)]">
                <template v-if="service.status === 'operational'">
