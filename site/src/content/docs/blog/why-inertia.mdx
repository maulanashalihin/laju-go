...
The same `useForm` pattern works in React and Vue — see [React + Inertia](/frontend/react-inertia/) and [Vue + Inertia](/frontend/vue-inertia/).
...
The initial HTML shell — the `<head>`, the asset tags, the root `<div data-page>` — is rendered by a [templ](./why-templ) component, not by a JS bundle. That means the first byte of HTML is real, crawlable, and fast. The frontend app hydrates into it. You get SPA navigation after the first load and a server-rendered shell on the first request, without running a Node SSR server.
