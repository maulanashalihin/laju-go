---
title: Introduction
description: Laju Go is a high-performance SaaS boilerplate built with Go Fiber, Svelte 5, React 19, or Vue 3, Inertia.js, SQLite, and templ.
---
...
- **Svelte 5 / React 19 / Vue 3 + Inertia.js** — SPA UX without writing a separate REST API — choose your frontend
...
<CardGrid stagger>
  ...
  <Card title="SPA without an API" icon="seti:svelte">
    Inertia.js drives Svelte 5, React 19, or Vue 3 pages from the server. `useForm` + `<form>` for CRUD — no `fetch` boilerplate, no client-side routing to maintain.
  </Card>
...
```mermaid
flowchart LR
    Browser[Svelte / React / Vue + Inertia] -->|HTTP| Fiber[Go Fiber]
